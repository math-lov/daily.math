 ...
 "lessons": [
  {
   ...
   "cards": [
    ...
    },
    {
     ...
     "body": {
      "zh": "十字相乘法用來分解 $ax^{2}+bx+c$（二次三項式）：把首項與末項各拆成兩個因數，排成兩欄，交叉相乘後相加，要等於中間項。\n在黑板上通常排成兩欄（左欄放首項的因數、右欄放常數項的因數，中間畫交叉線），並在右側寫出檢驗：\n　　檢驗中間項：$(2x)(+1)+(3x)(-5)=2x-15x=-13x$，與題目的中間項 $-13x$ 相同 ✓\n配對時要「橫向」取同一行的兩個數：第一行取 $2x$ 與 $-5$ 得 $(2x-5)$，第二行取 $3x$ 與 $+1$ 得 $(3x+1)$，所以 $6x^{2}-13x-5=(2x-5)(3x+1)$。\n草稿上的次序：先把左右兩欄的因數寫好 → 畫出交叉箭頭並算和 → 確認符號與數值無誤 → 最後才橫向配對寫出答案。"
     },
     "math": [
    ...
    },
    {
     ...
     "body": {
      "zh": "很多同學覺得十字相乘法「要試很多組合」。澳洲課本教另一個方法 —— 拆項法（split the middle term）：步驟固定，不用試，把中間項拆成兩項，再分組抽公因數。\n以 $6x^{2}-13x-5$ 為例：\n第 1 步：首項係數 × 常數項：$6\\times(-5)=-30$。\n第 2 步：找兩個數，相乘等於 $-30$、相加等於中間項係數 $-13$。試到 $(-15)\\times 2=-30$ 而且 $(-15)+2=-13$ ✓，就用 $-15$ 與 $2$。\n第 3 步：把中間項 $-13x$ 拆成 $-15x+2x$。\n第 4 步：分組抽公因數：$3x(2x-5)+1(2x-5)$，再抽 $(2x-5)$，得 $(2x-5)(3x+1)$。\n「兩個數的次序不重要」：寫成 $6x^{2}+2x-15x-5$ 也一樣 —— 分組後得 $2x(3x+1)-5(3x+1)=(3x+1)(2x-5)$，答案完全相同。\n兩種方法都可以用：十字相乘熟練後較快；拆項法步驟固定，首項係數較大時特別順手。"
     },
     "math": [
      "6\\times(-5)=-30",
