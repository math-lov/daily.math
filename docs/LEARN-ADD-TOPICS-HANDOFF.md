...
> **用途**：帶這份文件到**新的 chat**，繼續做 WS06 之後的課題（或 AS 評估）。
...
| 項目 | 數字 |
|---|---|
| 已完成課題 | ws01（**兩節**）、ws02、ws03、ws04、**ws05a（兩節）、ws05b**（同一份 WS05 拆成兩個課題） |
| MC 練習 | 110 題 |
| 長題示範 | 19 題（ws05a 5 條、ws05b 4 條；同一課 ≥4 條會收成一頁） |
| 概念卡 | 36 張（ws05b 的「複數除法：用共軛」是獨立一張） |
| SVG 示意圖 | 42 幅（25 個 id；只有 ws04 有圖） |
| 題解 | 129 份（每題一份） |
| 自編過渡題 | **22 題**（ws01 ×2 暖身、ws02 ×4、ws03 ×4、ws04 ×4、ws05a ×4、ws05b ×4） |
| 題目字眼（每課一組） | 6 課各自 5–6 組，全部為該課而設（見 §3.4） |
...
5. `data/learn/{bank,solutions,concepts}.json` —— **以 ws04（有圖）／ws05a、ws05b（最新）做範本**
6. `tools/make_learn_figures.py` —— SVG 圖產生器（`Frame`、`point`、`seg`、`arrow`、`arc`、`mirror_h/v`…）
7. `tools/learn_smoke_test.js` —— **硬編碼頁碼與課題數在這裡**（見 §6）
8. `data/learn/raw/` —— 來源抽取（**WS01–WS05、ASS1 已抽好**；下一份要抽的是 WS06，見 §3.0）
...
* 一堂課如果**概念卡 ≥ 7 張**或 **MC ≥ 18 題** → **拆成兩節**（範本：`ws01-1` 基礎 / `ws01-2` 進階）。
  〔老師也可以按節奏直接指定拆節：`ws05a` 只有 6 卡／17 MC（未達門檻），仍按老師要求拆成
...
* MC 盡量每頁 3 題；除不盡才用 2 題（如 ws03 最後兩頁、ws04 最後兩頁、ws05a 最後一頁）。
* 練習頁順序＝**過渡題在最前**（見 §4）。
...
* 已抽好的：`WS01–WS05`、`ASS1`；**WS06 之後全部要先抽**。
...
{ "id": "ws06", "stage": 1, "unit": 0, "subtopic": "…",
  ...
  "cmdHints": [                                   // ★ 這一課自己的「題目字眼」（見下方規則）
    { "en": "two distinct real roots", "zh": "兩個相異實根（$\\Delta>0$）" },
    { "en": "no real roots",           "zh": "沒有實根（$\\Delta<0$）" }
  ],
  "lessons": [{
...
`learn_smoke_test.js` 已鎖：**ws02–ws05b 的第一個練習頁必須全部是 `-w0\d` 題**
（新增課題時請照樣加同款斷言，見 §6.3）。
...
| 2 | **長題分數對不上** | WS1-EX1 總分 3 但 parts 1+1；EX2 總分 4 但 1+1。照官方 marking 抄：EX1＝(a)1A、(b)1M+1A；EX2＝(a)1A、(b)1M+1M+1A。**步驟分要逐題加起來＝`marks`**；`learn_smoke_test.js` 已對全部 19 個示範把關（`markSum`）。ws03／ws04 曾有多題少一分，已照 `raw/digest/WS03-sol.md`／`WS04-sol.md` 修好 |
...
* **`ok(home.$$(".topic-btn").length === 6, "home lists all six topics …")`** ←
  **再加課題就要同步改這個數**（現時 6：ws01、ws02、ws03、ws04、ws05a、ws05b）。
...
* **每課的「題目字眼」規則（§3.4）**：六課的 chips 要 ① 每課 4–6 個、② 每課至少 3 個獨有、
...
* **全站資料體檢（三個跨課題斷言，加內容後會自動把關）**：
  ① 每個示範的步驟分加起來＝該題 `marks`（19 個示範；`markSum`）；
  ② 沒有任何長題題幹以逗號結尾（「未完成的句子」）；
  ③ 整份課題資料不含 `**`（前端不支援 Markdown）。
* **多節課題**（ws01、ws05a）另有三類斷言：① `#pagenav .pg-lesson` 的數量與文字（「第 1 節」「第 2 節」）；
  ② 每節的練習頁形狀，例如 ws05a 是 `[[3,3,3],[3,3,2]]`；③ 每條示範都有自己的分頁格
  （該節示範 <4 條時不會收成一頁）。
* 一頁多條示範的切換器（`示範 1 / 4`、上一條／下一條、橫幅「下一條示範」）現在由 **ws05b** 把關
  （ws05a 拆節後每節只有 2–3 條）。改動那部分時記得別把這些斷言一起刪掉。

### 6.3 新增一課（WS06）時要加的斷言（照 ws05a／ws05b 那段抄）

1. 首頁課題數：`=== 6` → **`=== 7`**
2. `const t07 = boot("topic.html", "?t=ws06&p=0")` →
   課題名渲染、`#pagenav .pg` 頁數、概念卡有公式
3. 過渡題頁：第一個練習頁 `card[data-qid]` 全部符合 `/-w0\d$/`
4. 每頁 3 題 × 4 選項（`opt.length === 12`）
5. 有圖的話：作答前 0 幅、作答後每題至少 1 幅（＋長題逐步出圖）
6. 專屬「題目字眼」：把 `"ws06"` 加進 §6.2 那個 `["ws01", …, "ws05b"]` 清單，
   再補一條「ws06 有它自己的字眼」（例：`/two distinct real roots/`）
...
* **舊學生的進度百分比會回落**：加了過渡題令分母變大
  （ws01 18→20 題、ws02 14→18、ws03 18→22、ws04 12→16、ws05a 17、ws05b 16）。
...
這次要做：WS06（來源：inbox_learn/ 內的 EPH DSEL5 WS06 docx 與 _sol.docx）
做法：
 (1) 先照 handoff §3.0 抽取來源（extract_eph → wmf_to_png → eph_digest）
 (2) 照 §3.1–3.5 整理題目／題解／概念卡／編排／圖（以 ws04、ws05a 為範本）
 (3) 照 §3.4 為這一課寫 cmdHints（4–6 組、至少 3 組是這課獨有，先掃題目挑字眼）
 (4) 加 §4 的 4 題過渡題（放第一個練習頁）
 (5) 跑齊 §3.6 檢查，全綠才回報
 (6) 改 smoke test：§6.2 的「課題數 6 → 7」＋ §6.3 的新課斷言（含題目字眼那一條）
