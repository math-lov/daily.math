...
> ws01–ws04 的內容翻新已完成並**全部上線**，結構已獲老師確認「很好很滿意」→
> **不要改結構，照下面既有的 pattern 加內容就好。**
>
> 最後更新：2026-09-19 · 狀態：**已 push 並上線**（`main` ＝ `origin/main`，HEAD `b580877`）
...
* 網址：`https://math-lov.github.io/daily.math/learn/`（現時已是上述最新版）
...
4. `data/learn/lessons.json` —— 課程編排
5. `data/learn/{bank,solutions,concepts}.json` —— **以 ws04 做範本**（最新、有圖、有過渡題、欄位最齊）
6. `tools/make_learn_figures.py` —— SVG 圖產生器（`Frame`、`point`、`seg`、`arrow`、`arc`、`mirror_h/v`…）
7. `tools/learn_smoke_test.js` —— **硬編碼頁碼與課題數在這裡**（見 §6）
8. `data/learn/raw/` —— 來源抽取（**WS05 尚未抽取**，見 §3.0）
...
& $py tools\extract_eph.py --files WS05        # → data/learn/raw/WS05.json + WS05-sol.json
& $py tools\wmf_to_png.py --dir data\learn\raw\media --recursive   # 內的公式圖轉 PNG（讀圖轉寫要用）
& $py tools\eph_digest.py --files WS05 --variant both             # → raw/digest/WS05{,‑sol}.md（可讀摘要）
```

* 已抽好的：`WS01–WS04`、`ASS1`（WS01 起逐份抽）；**WS05 之後全部要先抽**。
...
```jsonc
{
 "id": "eph-ws05-q01",              // ⚠️ 必須 eph-<topic>-…（見 §5.1 第 5 條）
 "type": "mc",                       // mc 或 long
 "topic": "ws05",
...
```python
"eph-ws05-q01": [(my_fig_fn, "圖的說明（caption，顯示在圖下面）")],
...
或手動：`git add -A; git commit -m "Learn: add WS05"; git push`（GitHub Pages 約 1 分鐘部署；
...
`learn_smoke_test.js` 已鎖：**ws02/ws03/ws04 的第一個練習頁必須全部是 `-w0\d` 題**
（新增課題時請照樣加同款斷言，見 §6.3）。
...
### 5.2 ★ 三輪審閱最常挑出的 9 類問題（開工前先掃一次，可省一輪來回）

| # | 問題 | 具體案例／做法 |
|---|---|---|
| 1 | **id 前綴寫錯** | 曾用 `bridge-ws02-q01` → 前端認不出課題。一律 `eph-wsNN-…` |
| 2 | **長題分數對不上** | WS1-EX1 總分 3 但 parts 1+1；EX2 總分 4 但 1+1。照官方 marking 抄：EX1＝(a)1A、(b)1M+1A；EX2＝(a)1A、(b)1M+1M+1A |
...
> 這 9 類現在大部份已有自動防線（S2／S8／S10／S9／smoke 斷言），但**第 6、7 類仍要靠人手驗算**。
...
* **`ok(home.$$(".topic-btn").length === 4, "home lists all four topics …")`** ←
  **加了 WS05 就會 FAIL！** 必須改成 `=== 5`（或改為 `>= 4` 並改寫描述）。
...
### 6.3 新增一課（WS05）時要加的斷言（照 ws04 那段抄）

1. 首頁課題數：`=== 4` → **`=== 5`**
2. `const t05 = boot("topic.html", "?t=ws05&p=0")` →
...
1. **WS05–WS24**：每課照 §3（含 §3.0 抽取）＋ §4 過渡題。
...
* 已上線狀態：`main` ＝ `origin/main`（HEAD `b580877`），工作區乾淨。
...
這次要做：WS05（來源：inbox_learn/EPH_DSEL5_WS05_e.docx 與 _sol.docx）
做法：
 (1) 先照 handoff §3.0 抽取來源（extract_eph → wmf_to_png → eph_digest）
 (2) 照 §3.1–3.5 整理題目／題解／概念卡／編排／圖（以 ws04 為範本）
 (3) 加 §4 的 4 題過渡題（放在第一個練習頁）
 (4) 跑齊 §3.6 五道檢查，全綠才回報
 (5) 加了新課題，記得改 smoke test：§6.2 的「課題數 4 → 5」＋ §6.3 的新課斷言

⚠️ 開工前先掃 handoff §5.2「審閱最常挑出的 9 類問題」（特別是長題分數要對官方 marking、
   出題後要驗算至少一個選項正確、解說不可以自相矛盾）。
...
- [ ] `learn_smoke_test.js` **all learn smoke tests passed**（含改好的課題數／頁碼）
- [ ] 第一個練習頁是過渡題、題目難度由淺入深
