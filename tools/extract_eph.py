...
INBOX = os.path.join(BASE, "inbox_learn")
...
# ── EPH 檔名規則 ────────────────────────────────────────────────────────
FNAME_RE = re.compile(r"^EPH_DSEL5_(WS\d{2}|Ass\d+)_e(?P<sol>_sol)?\.docx$", re.I)
...
def discover() -> list[tuple[str, str, bool]]:
    ...
    for fn in os.listdir(INBOX):
        ...
        if not m:
            continue
        out.append((m.group(1).upper(), os.path.join(INBOX, fn), bool(m.group("sol"))))
